...
export const config = {
  ...
  troubleshootingUrl: env.TROUBLESHOOTING_URL ?? 'https://example.com/troubleshooting',
  useMockBackend: env.USE_MOCK_BACKEND !== 'false',
  mockOtpCode: env.MOCK_OTP_CODE ?? '1234',
  backend: {
    validateEmailUrl: env.VALIDATE_EMAIL_URL ?? 'https://example.com/validate-email',
    validateCodeUrl: env.VALIDATE_CODE_URL ?? 'https://example.com/validate-code',
    checkAccessUrl: env.CHECK_ACCESS_URL ?? 'https://example.com/check-access',
    policyUrl: env.POLICY_URL ?? 'https://example.com/policy',
    sendScanResultUrl: env.SEND_SCAN_RESULT_URL ?? 'https://example.com/send-scan-result'
  },
  timeoutsMs: {
